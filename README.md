...
点击悬浮“我想要”或按 F8，识别电脑正在播放的直播语音，将明确的纯数字口令填入点击时的输入框。每轮只填一次，不按回车发送。
...
**识别结果按数字口令处理：最终仅允许 ASCII `0–9`，共 1–32 位。** `008` 是三位口令，不会变成 `8`。字母模式已移除；小数点、正负号和其他符号不能写入。输入框已有的内容仍按原选区插入/替换，不擅自清空原内容。
...
- 取消、超时、迟到或重复的同轮结果不能再次写入；不同轮次里相同的数字仍可正常填写。
- 填写前核对目标窗口、控件、内容和选区；核对失败时仅保留有效结果供手动复制。
