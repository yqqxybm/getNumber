...
点击悬浮“我想要”或按 F8，识别电脑正在播放的直播语音，将明确“扣 / 飘”后的数字口令或四则运算结果填入当前输入框，或提交到已绑定的抖音网页版直播间。每轮最多一次；当前输入框模式不按回车。
...
实时接口只等“扣 / 飘”口令：出现完整数字/算式后的其他文字，就停止收音并请求最终转写；收到有效的已完成句也会立即结束本轮。不必等主播把后面的商品介绍讲完，也不因前面报价格后的停顿截断口令。中间文字只决定收音结束，不触发粘贴；最终转写仍可更正数字。没有明确口令时最长收音 12 秒，整轮最终结果也必须带“扣 / 飘”，否则不填写，整轮超时约 35 秒。HTTP 短音频接口没有中途转写，仍在讲话后约 650 毫秒安静时结束收音。直播音乐、断续口令和网络会影响实际响应时间。
...
6. 保持这个直播间标签页可见、弹幕框为空，点击悬浮 **我想要 / F8**。软件先检查目标，再收音；只有最终识别带“扣 / 飘”并通过纯数字与位数检查才填入并点击发送。
...
**只接受“扣 / 飘”引出的数字口令，最终仅允许 ASCII `0–9`，共 1–32 位。** `008` 是三位口令，不会变成 `8`。字母模式已移除；小数点、正负号和其他符号不能写入。当前输入框模式使用普通 Ctrl+V，由目标软件在光标或选区处粘贴；抖音模式要求弹幕框为空。
...
| 扣三个八 | `888` |
| 扣两个零，八 | `008` |
| 这件29，扣一个00 | `00`，忽略指令前的价格 |
| 库存18，飘一个零0九 | `009` |
| 这件30扣一个2乘3 | `6` |
| 29一件飘一个9全羊毛 | `9`，在“全羊毛”前结束 |
| 扣2加3乘4全羊毛 | `14`，先乘后加 |
| 飘9除3减1这个颜色好看 | `2` |
| 飘二乘以三 / 扣2×3 | `6` |
| 00 / 打个数字00 / 发00 | 缺少“扣 / 飘”，不填写 |
| 扣两个零三个八 | `00888` |
| 扣12 34 / 飘12，34 | `1234`，按数字口令分段报数合并 |
| 扣一百零二 | `102`，明确的中文数值转换 |
| 扣一百二 | 含义不明确，拒绝自动填写 |
| 扣两个12 / 扣两个一二 | 重复范围不明确，拒绝自动填写 |
| 扣12m3 | `12`，遇到 m 结束，不再取后面的 3 |
| 扣两个m / 扣O08 | 没有完整数字，不填写 |
| 扣1.2 / 扣-12 / 飘9.9元 | 拒绝，不把小数或负数截成正整数 |
| 不是扣123，是456 | 拒绝，不从否定或更正表达中擅自选择一个答案 |

处理顺序是：寻找第一个明确的“扣 / 飘”指令 → 读取完整数字或四则算式 → 遇其他内容结束 → 核对结果位数 → 在写入前再次校验。前面的商品介绍和价格不进入结果；口令后的“全羊毛”“元”“或者”等非算式文字也不进入结果，之后的数字和提示词不再采集。例如“扣9或者8”按这条边界规则取 `9`。没有“扣 / 飘”时，即使整段只有数字也不填写，超时和 HTTP 结果同样遵守这条规则。“不要扣00”“折扣00”“纽扣00”仍不能触发填写。

直播提示语只支持“飘、扣”，可带“请、大家”“一个/个”“数字”和冒号。例如“飘一个数字9/九” → `9`，“扣一个数字零零八” → `008`，“扣个数字一2三” → `123`。“一个数字”中的“一”不会进入结果；“飘两个9”按重复读法得到 `99`。

支持加、加上、+；减、减去、-；乘、乘以、×、*；除、除以、÷、/。先乘除、后加减，同级从左到右计算，最终只接受非负整数。例如“扣2加3乘4” → `14`、“扣9除3减1” → `2`。不完整算式、除零、负数结果、非整数结果、小数操作数和括号表达式不填写。单独“扣008”仍为 `008`；参与算式后按数值计算，结果不补前导零。
...
- 中间识别文字可触发停止收音；只有最终转写通过检查后才填写，因此不能保证零延迟。
- 取消、超时、迟到或重复的同轮结果不能再次写入；当前输入框模式的不同轮次可填写相同数字，抖音模式另有 30 秒重复保护。
- 当前输入框模式开始时绑定前台窗口和焦点，粘贴前重新核对；不要求目标支持 ValuePattern 直接写入。可读取的控件信息用于进一步核对，已知的密码、禁用或只读状态仍拦截。
- 结果写入剪贴板后执行一次 Ctrl+V，不按回车。数字留在剪贴板，原剪贴板内容会被替换。界面显示本轮目标与执行状态；“已执行一次粘贴”需在目标软件核对，不表示已发送消息。
- 请先点中微信、文本编辑器等软件的输入位置，再用 F8 或不抢焦点的悬浮按钮开始；保持目标不变。自绘控件可能无法提供光标/内容变化信息，因此不要在识别中继续编辑。受权限或快捷键限制的程序仍可能不响应，可手动粘贴。

底层按 [Microsoft SendInput 文档](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-sendinput) 一次提交 Ctrl+V 按键序列，检查已按下的修饰键和返回事件数。按键注入受 Windows 权限等级限制，不自动提权。
