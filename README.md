...
点击悬浮“我想要”或按 F8，识别电脑正在播放的直播语音，将明确“扣 / 飘”后的数字口令或乘法结果填入当前输入框，或提交到已绑定的抖音网页版直播间。每轮最多一次；当前输入框模式不按回车。
...
实时接口只等“扣 / 飘”口令，收到包含有效口令的已完成句就停止收音并处理结果，不再等整个会话结束，也不因前面报价格后的停顿截断口令。中间转写不触发粘贴；没有明确口令时最长收音 12 秒，整轮最终结果也必须带“扣 / 飘”，否则不填写，整轮超时约 35 秒。HTTP 短音频接口没有中途转写，仍在讲话后约 650 毫秒安静时结束收音。直播音乐、断续口令和网络会影响实际响应时间。
...
| 扣12m3 / 扣两个m / 扣O08 | 拒绝，不删字母凑数字、不猜字母对应数字 |
| 扣1.2 / 扣-12 / 飘9.9元 | 拒绝，不把小数、负数或价格变成口令 |
| 不是扣123，是456 | 拒绝，不从否定或更正表达中擅自选择一个答案 |

处理顺序是：寻找明确的“扣 / 飘”指令 → 完整解析指令后的数字表达 → 展开明确重复或计算乘法 → 核对位数 → 在写入前再次校验。指令前的商品介绍和价格不进入结果；没有“扣 / 飘”时，即使整段只有数字也不填写，超时和 HTTP 结果同样遵守这条规则。“不要扣00”“扣00或者11”“扣00再飘11”拒绝，不从多个目标中猜选。“折扣00”“纽扣00”不当作指令。

直播提示语只支持“飘、扣”，可带“请、大家”“一个/个”“数字”、冒号及句末的“吧、啊、呀”。例如“飘一个数字9/九” → `9`，“扣一个数字零零八” → `008`，“扣个数字一2三” → `123`。“一个数字”中的“一”不会进入结果；“飘两个9”按重复读法得到 `99`。“飘9元”“不要飘9”“飘9或者8”仍拒绝。

明确乘法支持“乘、乘以、×、*”，只接受完整的两个非负整数相乘的算式，结果也受位数限制。例如“扣2乘3” → `6`；“扣008”仍为 `008`，不会当成数值去掉前导零。不完整算式、小数、负数、除法或带单位的内容拒绝。实时接口须确认整条算式后才执行，不能把中间的“扣2”提前填入。
